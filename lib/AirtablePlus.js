...
class AirtablePlus {
    ...
    async delete(rowID, config) {
        const { tableName, base, complex, concurrency } = this._mergeConfig(config);
        let data = this._chunkData(rowID);
        
        let res = pMap(data, async items => {
            const records = await base(tableName).destroy(items);
