...
class AirtablePlus {
    ...
    async create(data, config) {
        ...
        return await this._handleBatch(data, cfg, async items => {
            const records = await this._throttle(() => base(tableName).create(items));
     ...
     * 
     * @param {Object} data Array of record objects to update.
     * @param {Object} [config] Optional configuration to override options passed into the constructor.
    ...
    async update(data, config) {
        ...
        return await this._handleBatch(data, config, async items => {
            const records = await this._throttle(() => base(tableName).update(items));
            return records.map(record => complex? record : record._rawJson);
        });
    }
     ...
     */
    async updateRow(rowID, data, config) {
        const { tableName, base, complex } = this._mergeConfig(config);

        const record = await this._throttle(() => base(tableName).update(rowID, data));
        return complex? record : record._rawJson;
    }
     ...
     * 
     * @param {Object[]} data Array of record objects to replace within Airtable.
     * @param {Object} [config] Optional configuration to override options passed into the constructor.
     * @returns {Promise<Object[]>} Array of record objects.
     * 
     * @example
     ...
     * }]);
     */
    async replace(data, config) {
        const cfg = this._mergeConfig(config);
        const { tableName, base, complex } = cfg;
        
        return await this._handleBatch(data, config, async items => {
            const records = await this._throttle(() => base(tableName).replace(items));
            return records.map(record => complex? record : record._rawJson);
        });
    }
     ...
     */
    async replaceRow(rowID, data, config) {
        const { tableName, base, complex } = this._mergeConfig(config);

        const record = await this._throttle(() => base(tableName).replace(rowID, data));
        return complex? record : record._rawJson;
    }
