...
/**
     ...
     * @param {boolean} [config.typecast] - Sets typecast on create and update operations for automatic data conversion
     */
class AirtablePlus {
    constructor(config) {
    ...
    async create(data, config) {
        ...
        const { tableName, base, complex, typecast } = this._mergeConfig(config);

        const record = await base(tableName).create(data, { typecast });
    ...
    async update(rowID, data, config) {
        ...
        const record = await base(tableName).update(rowID, data, { typecast });
        return complex? record : record._rawJson;
    }
